...
def walk(path, output):
    ...
    dirs = listdir(path)
    files = [r'{}'.format(os.path.join(path, f)) for f in dirs]
    for path in files:
...
def report_parallel(procs, total_time):
    ...
    for p in procs:
        ...
        print("PID: {}\t\t{} sec [{}] {}% [{} files]".format(p[0], round(p[1], 1), bar, perc.rjust(5), files_chunk))
    ...
    print("Total files by workers: {}".format(total_files))
    print("Folder with max files: \t{} [{} files]".format(folder_max_count, max_files))
    print("Folder with max size: \t{} [{}]".format(folder_max_bytes, bytes_human(max_bytes)))
    print("Avg time by workers: \t{} sec".format(round(avg, 2)))
    print("Difference (Max-Min): \t{}%".format(round(diff * 100, 2)))
    if diff > 0.3:
...
def get_directories(path, level=1):
    ...
    rest = []
    dirs = listdir(path)
    fulldirs = [r'{}'.format(os.path.join(path, f)) for f in dirs]
    files = []
    for f in fulldirs:
...
def main():
    ...
    if NP == 'MAX':
        ...
        pool = Pool(int(NP))
    N = len(pool._pool)
    print("Running with {} processes...".format(N))
    ...
    for line in rest:
        ...
        post_total += 1
    for p in procs:
        fname = '{}.{}'.format(os.environ['TEMP'], p[0])
    ...
    print("Pre-process:  \t\t{} sec".format(round(seconds_pre, 2)))
    worker_total = report_parallel(procs, seconds_parallel)
    print("Post-process: \t\t{} sec [{} files]".format(round(seconds_post, 2), post_total))
    total += post_total
    ...
    report = "Total files: \t\t{}\n".format(total)
    report += "Total time spent: \t{} sec ".format(round(total_seconds, 2))
    report += "[{}]\n".format(datetime.timedelta(seconds=total_seconds))
    report += "Rate: \t\t\t{} files/sec\n".format(int(total / total_seconds))
    report += "Output: {} [{}]\n".format(OUTPUT, bytes_human(os.lstat(OUTPUT).st_size))
    report += "/*******************************************************************/"
    print(report)
